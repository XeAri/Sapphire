...
size_t Core::ZoneServer::getSessionCount() const
{
...
bool Core::ZoneServer::createSession( uint32_t sessionId )
{
   auto pLog = g_fw.get< Core::Logger>();
...
void Core::ZoneServer::removeSession( uint32_t sessionId )
{
   m_sessionMapById.erase( sessionId );
}

Core::SessionPtr Core::ZoneServer::getSession( uint32_t id )
{
   //std::lock_guard<std::mutex> lock( m_sessionMutex );
...
Core::SessionPtr Core::ZoneServer::getSession( std::string playerName )
{
   //std::lock_guard<std::mutex> lock( m_sessionMutex );
...
void Core::ZoneServer::removeSession( std::string playerName )
{
